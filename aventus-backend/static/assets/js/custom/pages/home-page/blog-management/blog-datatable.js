...
var DatatablesServerSide = function() {
    ...
    var initDatatable = function() {
        ...
        dt = $("#company-profile-datatable").DataTable({
            ...
            columnDefs: [
                ...
                //         if(data == 'True'){
                //             label_badge_change = `<span style="cursor:pointer" data-id=${row.id}  class="btn btn-sm btn-outline btn-outline-dashed btn-outline-success btn-active-light-success popular_blog">Popular</span>`;
                //         }else if(data == 'False'){
                //             label_badge_change =  `<span style="cursor:pointer" data-id=${row.id}  class="btn btn-sm btn-outline btn-outline-dashed btn-outline-danger btn-active-light-danger popular_blog">Non Popular</span>`;
                //         }
    ...
    var PopularBlog = ()=>{
        const activeButton = document.getElementsByClassName('popular_blog')
        activeButton.forEach(d => {
            d.addEventListener('click', function(e) {
                const activeId = $(this).data('id');
                e.preventDefault();
                Swal.fire({
                    ...
                    if(result.value){               
                        $.post(`${api_config.popular_blog}`, {id:activeId }, function(data, status, xhr) {
                            if (data.status_code == 200) {
                                Swal.fire({
                                    text: "Successfully changed status ",
                                    icon: "success",
